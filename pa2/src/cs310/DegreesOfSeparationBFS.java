...
public class DegreesOfSeparationBFS {
    ...
    public static void main(String[] args) {
        ...
        String actor = "";
      //  String movie = "";
        LinkedStack<String> opprow = new LinkedStack<String>();
        ...
        while (!StdIn.isEmpty()) {
            ...
            sink = sink.substring(1, sink.length()-1);
            if (sg.contains(sink)) {
                int t = sg.indexOf(sink);
                if (bfs.hasPathTo(t)) {
                    for (int v : bfs.pathTo(t)) {
                        opprow.push(sg.nameOf(v));
                    }
                    actor = opprow.pop();
                    baconnumber = opprow.size() / 2;
                    StdOut.println(actor +" has a bacon number of "+ baconnumber );
                    while (!opprow.isEmpty()) {
                       StdOut.println(actor + " was in " + opprow.pop() + " with "
                       + (actor = opprow.pop()));
                    }
                }
                else {
