 ...
 *  @author Kevin Wayne
 */
  @SuppressWarnings({"unchecked"}) //added later

public class LinearProbingHashST2<Key, Value> {
    private static final int INIT_CAPACITY = 4;

    private int n;           // number of key-value pairs in the symbol table (active + inactive)
    private int m;           // size of linear probing table
    private Key[] keys;      // the keys
    private Value[] vals;    // the values
    private boolean[] flag;  // flag status
    private int inactive;    //number Key-value pair that is invalid
    /**
     ...
     */
    public int size() {
        return n-inactive;   //totak valid+invalid Key-Value pair - invalid Key-Value pair
    }
    ...
    public void put(Key key, Value val) {
        ...
        for (i = hash(key); keys[i] != null; i = (i + 1) % m) {
            if (keys[i].equals(key)) {
                vals[i] = val;
                if (!flag[i]) {   //if inactive in table
                  flag[i] = true;  //make it active and decrease inactive
                  inactive--;
                 }
                return;
            }
    ...
    public void delete(Key key) {
      ...
      //  keys[i] = null;
        //vals[i] = null;
         ...
         flag[i] = false; //already checked for contains
         inactive++;
    ...
    public Iterable<Key> keys() {
        ...
        for (int i = 0; i < m; i++)
            if (keys[i] != null && flag[i]) queue.enqueue(keys[i]);
        return queue;
    }
    ...
    public static void main(String[] args) {
      ...
      System.out.println("Before Deletion: size " + st.size());
      System.out.println("Before Deletion: n = " + st.n);
      System.out.println("Before Deletion: m = " + st.m);
      st.delete("S");
      st.delete("A");
      for (String s : st.keys())
          System.out.println(s + " " + st.get(s));

      System.out.println("After Deletion: size " + st.size());
      System.out.println("After Deletion: n = " + st.n);
      System.out.println("After Deletion: m = " + st.m);
    }
